...
async function _runTransition(onComplete, promptText) {
  ...
  if (!topBox) {
    ...
    Object.assign(divider.style, {
      ...
      height: "1px",
      background:
        "linear-gradient(90deg, transparent, var(--c-amber-1), transparent)",
      flexShrink: "0",
    });
  ...
  await new Promise((resolve) => {
    gsap
      ...
      .to(
        ".ft-vignette",
        { opacity: 0.6, duration: 0.8, ease: "power2.inOut" },
        "-=0.6",
      )
      .to(".ft-ember", { opacity: 1, duration: 0.4 }, "-=0.4")
      .to(
        ".ft-campfire",
        { scale: 1.3, y: 12, duration: 1.6, ease: "power1.inOut" },
        "-=0.1",
      );
  });
  ...
  for (let n = 3; n >= 1; n--) {
    ...
    playMP3(n === 1 ? "/audio/finalTick.mp3" : "/audio/buttonClick.mp3", {
      volume: 0.8,
    });
